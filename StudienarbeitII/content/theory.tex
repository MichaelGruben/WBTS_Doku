...
\begin{description}
  ...
  \item[2. Fortgeschrittener] Die Handlungen des Fortgeschrittenen sind
  gegenüber dem Novizen weniger kontextfrei. Sein weiterer Lernweg kann auf seiner kleinen
  ...
  \item[5. Meister] Ein Meister zeichnet sich gegenüber dem Experten zusätzlich
  zu fachlicher Kompetenz durch herausragende didaktische Fähigkeiten aus. Er
...
Für Masterly Mate bedeutet dies, dass das Aufsteigen in höhere Ränge
...
Zustand\footnote{ungeordnet und zufällig} des Bewusstseins steht im Flow ein
negentrischer Zustand\footnote{geordnet und vorhersehbar} gegenüber. Daher lässt
sich annehmen, dass eine Person im Flow sich auf ihrem höchsten Leistungsniveau befindet
...
Ganz nach dem Claim "`Fun is just another word for learning"'
\cite{koster:2005}, werden heute mithilfe von Gamification ernste Inhalte mit spielerischen
...
Anwendung überladen oder aufdringlich wirkt. So kann es sein, dass der
Anwender eine Übermotivation erfährt und keinen Grund mehr sieht aus eigenem
Antrieb heraus aktiv zu werden \cite{korte:2009}. Es muss also ein gesundes
...
handhabbares und frei verfügbares Instrument"' \cite{kreutzer:2011}. Damit
können sie ihre Werke mehr oder weniger frei verbreiten. Nutzer erhalten "`sehr weit
...
Einbindung von SCORM (siehe Abschnitt \ref{ref:scorm}) alle externen Varianten
und bietet demgegenüber kein eigenes Werkzeug zum Erstellen von Inhalten an.
...
virtuellem Klassenzimmer und \ac{CBT} auf. Das WBT kann demnach auf eine
tutorielle Betreuung verzichten und ist stark an die Verfügbarkeit in einem
...
nicht mit zu vielen Informationen oder kompliziertem Verhalten zu überfordern und somit zu frustrieren.
...
Der Mensch verfügt über ein breites Spektrum an Sinnesorganen. Mit diesen kann
er sich in seiner Umwelt zurechtfinden, um mit ihr interagieren zu können. Im
Bezug auf die Arbeit mit Software wird jedoch meist nur ein Teil der zur
Verfügung stehenden Sinne genutzt. Zu diesen zählt neben dem Hören vor allem das
Sehen, da heutzutage die Kommunikation der Maschine zum Menschnen über einen
Bildschirm realisiert wird.

Wie bereits erwähnt erhält der Mensch die Informationen, welche eine Maschine ihm 
zur Verfügung stellt meist über seinen visuellen Sinn. Damit Menschen diese
Informationen ohne größere Schwierigkeiten verarbeiten können, ist es wichtig
die einzelnen Objekte sinnvoll anzuordnen. Hierfür gibt es einige Grundsätze.

\subsubsection{Gesetz der Nähe}
Das menschliche Gehirn erkennt Gruppen von Objekten bereits ohne ihre
eigentlichen Eigenschaften zu kennen. Dies liegt daran, dass Objekte welche nahe
beieinander liegen, vom Gehirn als zusammengehörend identifiziert werden. Es
leuchtet daher ein, dass eine falsche Anordnung von Objekten dem Gehirn falsche
Zugehörigkeiten vorgaukelt. Der Mensch wird daher Objekte, die logisch
zusammengehören aber falsch positioniert wurden, auch an den falschen Stellen
suchen. Würde man z.B. auf einer Benutzeroberfläche, die Eingabefelder von Vor-
und Nachname an weit voneinander entfernte Positionen schreiben, so würden die
Benutzer eines der beiden Felder erst spät oder möglicherweise gar nicht
finden. Dies würde die Arbeit mit der Software erschweren und sie zäh und
ineffizient machen.
...
Objekte gleiche Farben in einem direkten Zusammenhang stehen müssen und wird sie
...
ihre Aufgaben effektiv und effizient erledigen können. D.h. die Software muss
...
Beispiele für eine solche Unterstützung wäre die Verwendung von Standardwerten
...
sollte ihn eine Software nicht verwirren indem sie altbekannte und weitgehend
...
eine serifenlose Schrift das Auge an. Bei Benutzeroberflächen hingegen wirken
...
folgenden vorgestellt \cite{johnson:2000}.
...
bei einer Authentifikation unumgänglich. 

\subsubsection{Identifizierung}
Die Identifizierung leitet den ersten Schritt einer Authentifikation ein. Dabei
wird der Nutzer dazu aufgefordert, seine eindeutige Benutzerkennung anzugeben. In vielen Systemen werden
...
\subsubsection{Beweis der Identifizierung}
...
welches auch als Authentifizierung bezeichnet wird. Man unterscheidet dabei
mehrere Authentifizierungsverfahren, die nach den Aspekten Wissen, Besitz und
biometrische Verfahren klassifiziert werden können.
...
\subsubsection{Vorbeugende Maßnahmen zur Sicherheit}
...
Für die Autorisierung können verschiedene Verfahren
...
Verfahren fallen dabei unter die einfachsten Zugriffsverfahren. 

Dabei werden einzelne Benutzer sogenannten Whitelisten hinzugefügt, um kenntlich
zu machen, welche Nutzer auf die entsprechende Ressource zugreifen können. Alle
Nutzer, die nicht in der Liste aufgeführt sind, besitzen demnach keinen Zugriff
auf die Ressource. Im Gegensatz zum Whitelisting werden beim Blacklisting die
Nutzer bzw. Subjekte aufgeführt, die keinen Zugriff auf eine Ressource erhalten.
Alle anderen Nutzer, die sich nicht auf der Liste befinden, erhalten einen
Zugriff. Das Whitelisting-Verfahren wird allerdings meist dem Blacklisting
bevorzugt, da die Anzahl der nichtberechtigten Nutzer auf eine Ressource in den meisten
...
Für die Zugriffskontrolle auf Dateisystemen werden heutzutage meist Matrixmodell
wie z.B. ACL\footnote{Access Control List} verwendet.
